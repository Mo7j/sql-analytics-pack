...
-- 2. What was the month/year combo for the first order placed?
SELECT DATE_TRUNC('day', occurred_at) AS month 
FROM orders
ORDER BY month;
...
SELECT AVG(standard_qty), DATE_TRUNC('month', occurred_at) AS month 
FROM orders
GROUP BY 2
ORDER BY 2;
...
SELECT AVG(gloss_qty), DATE_TRUNC('month', occurred_at) AS month 
FROM orders
GROUP BY 2
ORDER BY 2;
...
SELECT AVG(poster_qty), DATE_TRUNC('month', occurred_at) AS month 
FROM orders
GROUP BY 2
ORDER BY 2;

-- 6. The total amount spent on all orders on the first month that any order was placed in the orders table (in terms of usd).
SELECT SUM(total_amt_usd), DATE_TRUNC('month', occurred_at) AS month 
FROM orders
GROUP BY 2
ORDER BY 2
LIMIT 1;

-- Or to answer the questions from 3-6 in more simpler way 
...
-- NOTE: The SELECT DATE_TRUNC('month', MIN(occurred_at)) FROM orders will get the first month that have orders
...
-- Same solution, but using WIHT (cleaner)
WITH events AS (

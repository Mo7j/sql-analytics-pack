...
SELECT a.name account, s.name rep, r.name 
FROM accounts a
JOIN sales_reps s
ON a.sales_rep_id = s.id
JOIN region r
...
WHERE a.id = 1001;
-- 12. Find all the orders that occurred in 2015. Your final table should have 4 columns: occurred_at, account name, order total, and order total_amt_usd.
